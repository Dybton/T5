...
class TextToGraphQLDataset(Dataset):
  ...
  def __init__(self, tokenizer, type_path='train.json', block_size=102):
        ...
        for schema_path in schemas:
           with open(schema_path, 'r', encoding='utf-8') as s: ## Change
        ...
        with open(dataset_path, 'r', encoding='utf-8') as f:
          ...
          for element in data:
            question_with_schema = 'translate English to GraphQL: ' + element['question']  + ' ' + ' '.join(self.name_to_schema[element['schemaId']]) # Change
            tokenized_s = tokenizer.encode_plus(question_with_schema,max_length=1024, padding=True, truncation=True, return_tensors='pt') #Change
            self.source.append(tokenized_s)

            tokenized_t = tokenizer.encode_plus(element['query'],max_length=block_size, padding='max_length', truncation=True, return_tensors='pt') #Removed the tokens 
            self.target.append(tokenized_t)
            self.schema_ids.append(element['schemaId'])

  def get_question_with_schema(self, question, schemaId):
        return 'translate English to GraphQL: ' + question  + ' ' + ' '.join(self.name_to_schema[schemaId]) #Removed tokens
        ...
        with open(path, "r", encoding="utf-8") as f:
          ...
          for example in data:
            ...
            for pos in range(1, token_count):
              ...
              if encoded_target is not None and torch.numel(encoded_target) > 0: #Change - why was this changed?
  ...
  def __init__(self, tokenizer, type_path='train_spider.json', block_size=102):
        ...
        with open(path, 'r') as f, open(tables_path, 'r') as t:
          ...
          for element in data:
            ...
            db_with_question = 'translate English to SQL: ' + element['question'] + ' ' + tables_with_columns #Change
          ...
          for element in data:
            for interaction in element['interaction']:
              ...
              for pos in range(1, token_count):
                ...
                encoded_source[pos] = tokenizer.mask_token_id
                decoded_target = ''.join(tokenizer.convert_ids_to_tokens([target_id])) #Change
...
class T5MultiSPModel(pl.LightningModule):
  def __init__(self, hyperparams, task='denoise', test_flag='graphql', train_sampler=None, batch_size=2,temperature=1.0,top_k=50, top_p=1.0, num_beams=1 ):
    ...
    self.hyperparams = hyperparams #change names
  ...
  def forward(
    self, input_ids, attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None, labels=None #change names
    ):
    return self.model(
        input_ids,
  ...
  def configure_optimizers(self):
    t_total = len(self.train_dataloader()) * self.trainer.max_epochs * self.trainer.limit_train_batches #change
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=self.hyperparams.lr, eps=1e-8) #chagnes
  ...
  def exact_match_accuracy(self, preds, target): # Exact match function
    total = len(preds)
  ...
  def test_epoch_end(self, outputs):
    ...
    self.log("avg_test_loss", avg_loss, prog_bar=True) # Change

    if self.test_flag == 'graphql':
        avg_accuracy = torch.stack([x["test_accuracy"] for x in outputs if "test_accuracy" in x]).mean() #Custom logger
  ...
  def train_dataloader(self):
    return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, collate_fn=self.custom_collate_fn, num_workers=32) #change

  def val_dataloader(self):
    return DataLoader(self.val_dataset, batch_size=self.batch_size, collate_fn=self.custom_collate_fn, num_workers=32) #change
